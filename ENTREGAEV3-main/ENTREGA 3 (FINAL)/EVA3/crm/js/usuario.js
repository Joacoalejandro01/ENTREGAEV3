...
var password_usuario = document.getElementById("txt_usuario_password").value;
  ...
  if (celular_usuario === ""|| isNaN(parseInt(celular_usuario))) {
    mostrarAlerta('danger', 'Ingrese un dato correcto. Por favor, revise los datos.');
    return;
  }
  if (username_usuario === "") {
...
function actualizarUsuario(){
  ...
  .then((response) => {
    if (response.status == 200) {
      ...
      mostrarAlerta('success', 'Usuario eliminado exitosamente.'); // Mensaje de éxito
    } else if (response.status == 400) {
      mostrarAlerta('danger', 'No se pudo eliminar el usuario.'); // Mensaje de error
    }
  })
  .then((result) => console.log(result))
  .catch((error) => console.error(error));
